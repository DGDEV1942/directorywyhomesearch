...
<div id="main">
	...
	<form id="create-listing" method="POST" action="<?php echo appthemes_get_step_url(); ?>">
						...
						<div class="plan">
							<div class="content">
								...
								<?php } else if( 'forced_recurring' == $recurring) { ?>
									<p class="recurring-description"><?php _e( '* This plan includes recurring pricing. Your account will be automatically charged each month.', APP_TD ); ?></p>
								<?php } ?>
							...
							<div class="price-box">
								...
								<div class="duration">
									<?php if( $plan['duration'] != 0 ) {
										...
										if( ( $period == 'every<br />' && $period_type == 'month' ) || ( $period == 'every 30<br />' && $period_type == 'days' ) ) {
											print( 'per<br />month' );
										} else {
		...
		<fieldset>
			...
			<input type="checkbox" id="accept-toc" name="accept-toc">
			<label for="accept-toc">I have read and understand the <a href="/terms" target="_blank">Terms and Conditions</a></label>
			<div class="form-field">
				<?php if( !empty( $plans ) ){ ?>
					<input type="submit" disabled value="<?php _e( 'Continue', APP_TD ) ?>" />
				<?php } ?>
			</div>
		</fieldset>
	</form>
