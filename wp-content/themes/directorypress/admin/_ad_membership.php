...
<fieldset><div class="titleh"> <h3>Package Display Options</h3></div>
...
<option value="yes" <?php if($packagedata['show_register'] == "yes"){ echo "selected=selected"; } ?>>Display</option>
...
<option value="yes" <?php if($packagedata['show_myaccount'] == "yes"){ echo "selected=selected"; } ?>>Display</option>
...
<fieldset><div class="titleh"> <h3>Membership Transfer</h3></div>
...
<div class="ppt-form-line">	<p><input class="premiumpress_button" type="submit" value="Transfer" style="color:#fff;" /> </p></div>
...
<input type="hidden" name="ppt_membership[package][ID][]" value="<?php echo $package['ID']; ?>" />
...
onclick="PPMsgBox(&quot;<strong>What is this?</strong><br /><br />This is where you choose how many days (numeric value) before the package expires. Once expired the users membership will be removed. &quot;);"><img src="<?php echo PPT_FW_IMG_URI; ?>help.png" style="float:right;" /></a></span>
 <input name="ppt_membership[package][duration][]" type="text" class="ppt-forminput" style="width:50px;" value="<?php echo $package['duration']; ?>" /> Days
<div class="clearfix"></div>
</div>
...
<div class="clearfix"></div>
<input  type="checkbox" name="checkme1<?php echo $i; ?>" value="1" onchange="ChangeTickValue('checkme1<?php echo $i; ?>');" <?php if($package['recurring'] == 1){ echo "checked=checked"; } ?> style="margin-left:140px;" />Recurring Payment (PayPal Only)
...
<a href="javascript:void(0);" onclick="jQuery('#packagebox').clone().appendTo('#PACKAGEDISPLAYHERE');" class="button tagadd" style="float:right;">Add New Package</a>
