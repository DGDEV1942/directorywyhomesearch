...
$params['sql_query']				= 'SELECT order_id,order_date,cus_name,order_total,order_status FROM '.$wpdb->prefix.'orderdata WHERE order_status='.$_GET['status'];
	...
	if(!isset($_get['ajaxcall'])){
	// $params['sql_query'] .= ' WHERE order_id !="" ORDER BY order_id desc';
	}
}
...
<div id="premiumpress_tab1" class="content"> <fieldset style="padding:0px;"><?php echo $out;?></fieldset>




<a href="admin.php?page=orders&exportcsv=1" style="padding:5px; margin:20px; margin-left:0px; background:green; color:#fff;">Export Sales To CSV</a>
