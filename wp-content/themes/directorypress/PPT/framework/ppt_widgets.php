...
class PPT_Widgets_TAXOMONY extends WP_Widget {
    ...
    function PPT_Widgets_TAXOMONY() {
		...
		$opts = array(
			...
			'description' => __( 'Here you can display any of your custom taxonomies.' )
		);
  ...
  <tr>
	...
	foreach($taxArray as $tax){
		...
		if(in_array($tax['name'],$instance['taxID'])){
    		...
    		echo "<div class='boxme'> <h3>".$tax['title']."</h3>";
		    ...
		    $terms = get_terms($NewTax);
			 $count = count($terms);
			 if ( $count > 0 ){
				 echo "<ul id='ppt_taxonomy_widget_".$NewTax."list'>";
				 foreach ( $terms as $term ) {
			 
				   echo "<li><a href='".get_term_link($term->slug, $NewTax). "'>" . $term->name . "</a></li>";					
				 }
				 echo "</ul>";
			 }
	...
	if(isset($instance['adsearch']) && $instance['adsearch'] == 1){
		...
		echo '<a  href="javascript:document.AdvancedSearchTaxForm.submit();" style="margin-top:10px;float:right;margin-right:20px;" > 
...
class PPT_Widgets_PriceSearch extends WP_Widget {

    function PPT_Widgets_PriceSearch() {
        // widget actual processes		
		$opts = array(
...
<option <?php if($instance['order'] == "order"){ echo "selected=selected"; } ?>>order </option>
	...
	echo $PPTDesign->FeaturedStores($instance['storeID'],$instance['order']);
...
class PPT_Widgets_FeaturedCategories extends WP_Widget {

    function PPT_Widgets_FeaturedCategories() {
        // widget actual processes		
		$opts = array(
			'classname' => 'ppt-FeaturedCategories',
			'description' => __( 'Search box for your website.' )
		);
		parent::__construct( 'ppt-FeaturedCategories', __( 'NEW ** FEATURED CATEGORIES **' ), $opts );
  ...
  <div class="msg msg-info"> 
	 <p style="background:none;">To edit the category images use the <a href="admin.php?page=setup" style="font-weight:bold; text-decoration:underline;">general setup -> categories</a> tab.</p>
    </div> 
	...
	<input class="widefat" type="text" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo esc_attr( $instance['title'] ); ?>" /> 
     ...
     <p><br /><b>Select Categories to Display</b> (Hold Shift+Ctrl to select multiple)</p>  
      <select id="<?php echo $this->get_field_id( 'storeID' ); ?>" name="<?php echo $this->get_field_name( 'storeID' ); ?>[]" multiple="multiple" style="width:100%; height:150px;">
      <?php echo premiumpress_categorylist($instance['storeID'],false,false); ?>
    </select> 
...
<option <?php if($instance['order'] == "order"){ echo "selected=selected"; } ?>>order </option>
	...
	echo $before_widget.str_replace("widget-box-id","ppt-widget-featuredcategories",$before_title).$instance['title'].$after_title.'<div id="ppt-widget-featuredcategories-box" ><ul>';
	echo $this->FeaturedCategories($instance['storeID'],$instance['order']);
	echo '</ul><div class="clearfix"></div></div>'.$after_widget; 

	}

	function FeaturedCategories($storesID="",$order=""){
	
		global $wpdb,$PPT; $STRING = "";
		
		$showcount = get_option('display_categories_count');
		...
		foreach($categories as $category) {	
			...
			$IMAGE = $PPT->CategoryExtras($category->cat_ID,"image",1);	
				
			$STRING .= '<li>';		
					
			$STRING .= "<a href='".get_category_link( $category->term_id )."' title='".$category->cat_name."' style='text-decoration:none;' ";
...
class PPT_Widgets_SEARCH extends WP_Widget {
    ...
    function PPT_Widgets_SEARCH() {
		...
		parent::__construct( 'ppt-search', __( 'NEW ** SEARCH WIDGET ** ' ), $opts );
	...
	echo $before_widget.str_replace("widget-box-id","ppt-widget-search",$before_title).$instance['title'].$after_title.'<div id="ppt-widget-search-box" >';
     ...
     <a  class="button gray floatr" onclick="document.quickSearchBox.submit();"><?php echo $instance['btn']; ?></a>  
            ...
            <input type="text" value="<?php echo $instance['default']; ?>" name="s" id="s" onfocus="this.value='';" style="width:130px;"  />
            
            <?php if(get_option("display_advanced_search") ==1){ ?><a href="javascript:jQuery('#AdvancedSearchBox').show(); javascript:void(0);"><small><?php echo $PPT->_e(array('head','3')) ?></small></a><?php } ?>
...
<p><br /><b>Button Icon</b></p> 
 ...
 <p><br /><b>Button Shape</b></p> 
	...
	global $PPT,$PPTDesign; $STRING = ""; extract($args);
	 ...
	 echo '<a href="'.$instance['link'].'" class="button '.$instance['size'].' '.$instance['color'].' '.$instance['shape'].'" style="'.$instance['css'].'">';
...
class PPT_Widgets_FEATURED extends WP_Widget {
    ...
    function PPT_Widgets_FEATURED() {
		...
		$opts = array(
			...
			'description' => __( 'Display a list of featured listings.' )
		);
		parent::__construct( 'ppt-featured', __( 'NEW ** FEATURED LISTINGS **' ), $opts );
    ...
    <p class="ppnote" style="background: #DBFFDE;
padding: 10px;
color: #078B10;
min-width: 280px;
font-size: 10px;
border: 1px silid #DDD;"><b>Note:</b> Only listings which have 'featured' set to 'yes' will be included in the search results. Edit any product and set the product to featured if you are unsure.</p>
	...
	<p><b>From Which Category?</b></p>
 	...
 	<select id="<?php echo $this->get_field_id( 'cat' ); ?>" name="<?php echo $this->get_field_name( 'cat' ); ?>"  style="width: 240px;  font-size:14px;">
...
<?php
	...
	global $PPT, $PPTDesign, $wpdb; $STRING = ""; extract($args); $ex = ""; if(!isset($GLOBALS['thisID'])){ $GLOBALS['thisID']=0; }
	...
	$cat = $instance['cat'];
 	...
 	// trying to be celever confuses people elseif(is_numeric($GLOBALS['premiumpress']['catID'])){		$ex = "&cat=".$GLOBALS['premiumpress']['catID'];	}
	if($cat != ""){		$ex = "&cat=".$cat; 	}else{		$ex =""; 	}
		 
	$posts = query_posts('posts_per_page='.$num.'&meta_key=featured&meta_value=yes&orderby=rand'.$ex); 
	...
	if(count($posts) > 0 ){ //meta_value_num
		...
		foreach($posts as $post){ 
			...
			if( $post->ID == $GLOBALS['thisID'] && count($posts) == 1){
			
			$STRING =""; 
			
			}else{ 
						  
				$STRING .= '<div class="featureditem clearfix">'; 
				
				$STRING .= '<div class="imgb">';
				...
				if(strtolower(PREMIUMPRESS_SYSTEM) == "resumepress"){
					...
					if($img == ""){
						...
						$img = "<img src='".get_option('imagestorage_link').$img."' class='photo' alt='user ".$post->post_author."' />";
					}
				
				$STRING .= $img;
				}else{
				
				$injectimg = premiumpress_image($post->ID,"",array('alt' => $post->post_title, 'link' =>true, 'link_class'=>'frame right', 'width' => '50', 'height' => '50', 'style' => 'max-width:80px;' ));
				 
				}
				  
				
				$STRING .= '</div><div class="contentb">'.$injectimg;
				
						$STRING .= '<h3><a href="'.get_permalink($post->ID).'" title="'.$post->post_title.'">'.$post->post_title.'</a></h3>';
						
						$STRING .= '<p>'.substr(strip_tags($post->post_excerpt),0,100).'..</p>';
						
						if(strtolower(PREMIUMPRESS_SYSTEM) == "auctionpress"){
						
						$STRING .= '<p class="fbid">
					 	<div style="float:right;">'.$PPTDesign->TimeDiff(date('Y-m-d H:i:s',strtotime(date("Y-m-d H:i:s", strtotime($post->post_date)) . " +".get_post_meta($post->ID, "expires", true)." days")),4).'</div>
						<b> '.premiumpress_price(get_post_meta($post->ID, "price_current", true),$GLOBALS['premiumpress']['currency_symbol'],$GLOBALS['premiumpress']['currency_position'],1).'</b>
						</p>';					 
						
						}elseif(strtolower(PREMIUMPRESS_SYSTEM) == "shopperpress"){
						
						$STRING .= '<p>
						<strike>'.premiumpress_price(GetPrice(get_post_meta($post->ID, "old_price", true)),$_SESSION['currency_symbol'],$GLOBALS['premiumpress']['currency_position'],1).'</strike> 
						<b>'.premiumpress_price(GetPrice(get_post_meta($post->ID, "price", true)),$_SESSION['currency_symbol'],$GLOBALS['premiumpress']['currency_position'],1).'</b>
						</p>';					 
...
class PPT_Widgets_ARTICLES extends WP_Widget {
 ...
 <p><br /><b>Which Article?</b> (Hold Shift+Ctrl to select multiple)</p>  
  ...
  $posts = query_posts('post_type=article_type&posts_per_page=100&orderby=ID&order=DESC');	 foreach($posts as $post){ ?>
...
<?php
	...
	if(!is_array($ID)){ return; }
	 	...
	 	foreach($ID as $val){ $n .= $val.",";}
	 
	 if(empty($ID)){  $doThis = array( 'post_type' => 'article_type', 'posts_per_page' => 1 ); }else{  $doThis = array( 'post_type' => 'article_type', 'post__in' => explode(",",$n), 'posts_per_page' => count(explode(",",$n))  ); }
	 
	 
		$a = query_posts($doThis);		
 
		foreach($a as $post){
		
			   $dd = "<div class='time'>".get_the_time('l, F jS',$post)."</div>";  
			...
			if($details){
				...
				$STRING  .="<li>".premiumpress_image($post->ID,"",array('alt' => $post->post_title, 'link' =>true, 'link_class'=>'frame right','width' => '50', 'height' => '50', 'style' => 'auto' ))."<h3><a href='".get_permalink( $post->ID )."'>".$post->post_title."</a></h3>".$dd."".$cont."</li>";			
			 
			}else{ 
			
			
				$STRING  .="<li><a href='".get_permalink( $post->ID )."'>".$post->post_title."</a>".$dd."<div class='clearfix'></div></li>";			
 ...
 class PPT_Widgets_CATEGORIES1 extends WP_Widget {

    function PPT_Widgets_CATEGORIES1() {
        // widget actual processes		
		$opts = array(
			'classname' => 'ppt-categories1',
			'description' => __( 'Display website categories in your sidebar using a toggle effect.' )
		);
		parent::__construct( 'ppt-categories1', __( 'NEW ** ALL CATEGORIES TOGGLE **' ), $opts );
	...
	?>
 <div class="msg msg-info"> 
	 <p style="background:none;">To edit the category display order change the settings under the <a href="admin.php?page=setup" style="font-weight:bold; text-decoration:underline;">general setup -> categories</a> tab.</p>
    </div> 
     
	<p><b>Box Title:</b></p>
	<input class="widefat" type="text" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo esc_attr( $instance['title'] ); ?>" />
  
     <p><br/><b>Category Display:</b></p>
 
    <select id="<?php echo $this->get_field_id( 'type' ); ?>" name="<?php echo $this->get_field_name( 'type' ); ?>" style="width: 220px;  font-size:14px;">
				<option value="post" <?php if(esc_attr( $instance['type'] ) =="post"){ print "selected";} ?>>Product/Listing Categories</option>
...
<?php
	...
	if(isset($GLOBALS['IS_CHECKOUTPAGE'])){ return; }	
	 ...
	 if($instance['type'] == "post"){
	 	...
	 	$STRING .= $this->SYS_CATEGORIES(true); 
	 	 
	 }else{
	  
		if($instance['type'] == ""){ $tty="article"; }else{ $tty = $instance['type']; }
		...
		$fcats .= $cat->cat_ID.",";            
        $STRING .= '<div><a href="'.get_term_link( $cat,$cat->taxonomy  ).'" title="'.$cat->category_nicename.'">'.$cat->cat_name.'</a></div>';
        } 	 
	 }
...
function SYS_CATEGORIES($full=false){
	...
	/************************/
		...
		if(strlen($DISPLAYCATS) > 1 && !$full){$MAKEDISPLAY = "&include=".$DISPLAYCATS; }
		if($full){ $MAKEDISPLAY .= "&hierarchical=1"; }
		
	  $sql = 'pad_counts=1&use_desc_for_title=1&hide_empty=0&hierarchical=0'.$MAKEDISPLAY;
  
		 $cats = get_categories($sql);
		 $cats_count = count($cats);
		...
		foreach($SHOWRESULTS as $ThisCat){  if( ( isset($ThisCat['ID']) && $ThisCat['ID'] > 0 && $MAKEDISPLAY != "" ) || ( $MAKEDISPLAY =="" ) ){	
		 ...
		 if( strlen($cat->cat_name) > 1 && ($MAKEDISPLAY =="" && !in_array($cat->term_id,$DONECATS) ) || ( isset($ThisCat['ID']) && $cat->term_id == $ThisCat['ID'] ) ) {
			...
			$cats_sub_count = count($cats_sub); 
		 	...
		 	$string .= '<div class="trigger"><a href="#" title="'.$cat->category_nicename.'">';
			$string .= $cat->cat_name;
			if($SHOWCATCOUNT =="yes"){ $string .= " (".$cat->count.')'; }
			$string .= '</a></div><div class="container">';
...
class PPT_Widgets_CATEGORIES extends WP_Widget {
    ...
    function PPT_Widgets_CATEGORIES() {
		...
		$opts = array(
			...
			'description' => __( 'Display website categories in your sidebar with a fly-out effect.' )
		);
		parent::__construct( 'ppt-categories', __( 'NEW ** ALL CATEGORIES **' ), $opts );
...
<?php
	...
	if(isset($GLOBALS['IS_CHECKOUTPAGE'])){ return; }	
	 ...
	 $STRING .= '<ul class="category vertical" id="Accordion">';
	 ...
	 if($instance['type'] == "post"){
		...
		$fcats .= $cat->cat_ID.",";            
        $STRING .= '<li><a href="'.get_term_link( $cat,$cat->taxonomy  ).'" title="'.$cat->category_nicename.'">'.$cat->cat_name.'</a></li>';
        } 	 
	 }
...
/******************************************************************************/
  ...
  function widgets_init() {
	...
	register_widget( 'PPT_Widgets_CATEGORIES1' );	
	register_widget( 'PPT_Widgets_FEATURED' );
	register_widget( 'PPT_Widgets_BUTTON' );
	register_widget( 'PPT_Widgets_SEARCH' );
	register_widget( 'PPT_Widgets_FeaturedCategories' );	
	register_widget( 'PPT_Widgets_PriceSearch' );
