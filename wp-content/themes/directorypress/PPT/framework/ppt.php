...
function ppt_query_string($query_string) { 
	...
	// ONLY PERFORM THIS QUERY IF ITS FOR THE CATEGORY PAGE
	if( substr($query_string,0,13) == "category_name" || isset($_GET['orderby']) ){
	...
	$query_string = str_replace("Enter%20a%20keyword..","",$query_string);
...
global $wpdb;
	...
	if(strpos($arg,"INNER JOIN $wpdb->postmeta") !== false){		 
		return $arg;
	}
...
function query_orderby($sort) {
	...
	switch($ppt_default_orderby){
		...
		case "meta_value&meta_key=price*desc":
		case "meta_value&meta_key=packageID*desc":
		case "meta_value&meta_key=featured*desc": { if(isset($_GET['s']) || isset($_GET['search-class']) ){ $sort = str_replace("$wpdb->posts.post_date DESC","$wpdb->postmeta.meta_value DESC",$sort); } } break;
		
		case "meta_value&meta_key=price*asc":		 
		case "meta_value&meta_key=packageID*asc": { if(isset($_GET['s']) || isset($_GET['search-class']) ){ $sort = str_replace("$wpdb->posts.post_date DESC","$wpdb->postmeta.meta_value ASC",$sort); }  } break;
...
function ppt_last_login($login) {
	...
	global $post, $pagenow, $page;	
		...
		if($pagenow == "wp-login.php" && $_GET['action'] != "logout" && $_GET['action'] != "rp" && $_GET['action'] != "resetpass"){
			...
			echo "<script type='text/javascript' src='".$GLOBALS['bloginfo_url']."/wp-includes/js/jquery/jquery.js?ver=1.7.1'></script>";
...
if(is_array($layouttypes) && !empty($layouttypes)){	
	...
	echo "#header { background:".$bgc." ".$layouttypes['header']['bg']." url('".$layouttypes['header']['image']."') ".$layouttypes['header']['image-repeat']."; 
	".$layouttypes['header']['custom']." }"; } ?>
	...
	.menu li ul, #hpages li ul a, #hpages li ul a, .submenu li ul a, .category li ul a, .submenu_account li ul a   { background: #<?php echo $layouttypes['nav']['dropbg']; ?>;  }
	<?php } ?>
	<?php if(isset($layouttypes['nav']['dropbgh']) && strlen($layouttypes['nav']['dropbgh']) > 1){ ?>
	.menu li ul a:hover, #hpages ul li ul a:hover, .submenu li ul a:hover, .category li ul a:hover, .submenu_account li ul a:hover  { background: #<?php echo $layouttypes['nav']['dropbgh']; ?>;  }
	<?php } ?>
	...
	.itembox h2,.itembox h1 {   color: #<?php echo $layouttypes['itembox']['text']; ?>; 	
	...
	.featuredlisting { background: #<?php echo $layouttypes['gallery']['featured-bg']; ?> !important;
