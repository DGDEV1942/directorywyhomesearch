...
function start(){
	...
	if(isset($_POST['action'])){
			...
			$message = "<p> Name : " . strip_tags($_POST['form']['name']) . "</p>
						<p> Email : " . strip_tags($_POST['form']['email']) . "</p>
						<p> Message : " . strip_tags($_POST['form']['message']) . "</p>
						<p>(sent via the mobile contact form)</p>";
...
function _check(){
		...
		// CHECK FOR IPAD 		
		if (strpos(strtolower($agent), "ipad") === false) { }else{ return false;} 
		// CHECK FOR FACEBOOK
...
global $PPT;
	...
	<style type="text/css">.menu{color:#FFFFFF; background:url('.PPT_FW_IMG_URI.'mobile/header_bg.png) repeat-x top;} a { color:#8e0000; } 
...
ul.paginationD01 li a:hover,ul.paginationD01 li a.current{color:#893A00;text-shadow:0px 1px #FFEF42;border-color:#FFA200;background:#FFC800;background:-moz-linear-gradient(top,#FFFFFF 1px,#FFEA01 1px,#FFC800);background:-webkit-gradient(linear,0 0,0 100%,color-stop(0.02,#FFFFFF),color-stop(0.02,#FFEA01),color-stop(1,#FFC800));}
 ...
 ul.paginationD10 li a:hover,ul.paginationD10 li a.current{text-shadow:0px 1px #3C3C3C;border-color:#202020;background:#525252;background:-moz-linear-gradient(top,#9F9F9F 1px,#6C6C6C 1px,#525252);background:-webkit-gradient(linear,0 0,0 100%,color-stop(0.02,#9F9F9F),color-stop(0.02,#6C6C6C),color-stop(1,#525252));}
	...
	<style type="text/css">
	.f_half { width: 40%; padding-bottom:1px; float:left; }
	</style>
	';
	
	if(strtolower(PREMIUMPRESS_SYSTEM) == "moviepress"){
	$STRING .= '<script src="'.PPT_THEME_URI.'/PPT/js/jquery.swfobject.js" type="text/javascript"></script>';
	}
	...
	<body onload="setTimeout(function() { window.scrollTo(0, 1) }, 100);" />
        	...
        	<div class="box_content"> 
			    ...
			    <select class="form_input_select" name="cat"><option value="">---------</option>'.premiumpress_categorylist().'</select>  
...
function _body(){
	...
	global $PPT, $PPTDesign, $wpdb, $post,  $query_string, $ThemeDesign, $userdata; get_currentuserinfo();	
	...
	switch($_GET['show']){
		...
		case "search":{
			...
			if(count($a) > 0){
				...
				'.premiumpress_image($post->ID,"",array('alt' => $post->post_title,  'link' => false, 'link_class' => 'frame', 'width' => '50', 'height' => '50', 'style' => 'auto' )).'
				</a>			 
				</div>
				...
				$STRING .= ' <ul class="pagination paginationD paginationD10">'.$PPTDesign->PageNavigation(true).'</ul>';
		...
		case "single": {		
			...
			if(strtolower(PREMIUMPRESS_SYSTEM) == "moviepress"){	
				...
				$GLOBALS['membershipPackage'] = $userdata->yim['user_package'];
				...
				$extra = str_replace('960','270',str_replace('518','180',$extra)); 
			}
        	...
        	<img src="'.premiumpress_image($post->ID,"url",$imgExtra).'" width="50" height="50" class="left_pic"/>
			...
			<div class="services_content">				
				...
				<p>Visitors: '.$hits.'</p>				 
				'.$PPTDesign->CustomFields($post,get_option("customfielddata"),true).'<div class="clear"></div>';
...
.subtext { font-size:10px;  min-height:20px; }
...
return $STRING;
		...
		while($tt > 0){  
		 ...
		 $image_array = explode(",",$images); 
			...
			foreach($image_array as $image){ if(strlen($image) > 2){ 
				...
				$string .= '<img alt="'.premiumpress_image_check($image,"full").'"  style="opacity: 1; " src="'.premiumpress_image_check($image,"t","&amp;w=50&amp;h=50").'" alt="image" />';
	...
	switch(strtolower(PREMIUMPRESS_SYSTEM)){
		...
		case "shopperpress": {  
			...
			$oldPrice = premiumpress_price($op,get_option('currency_code'),get_option('display_currency_position'),1); ;
			$Price = premiumpress_price(get_post_meta($post->ID, "price", true),get_option('currency_code'),get_option('display_currency_position'),1); ;
