...
class PremiumPressTheme_Payment {
	...
	**/
	...
	global $wpdb, $userdata; get_currentuserinfo(); $returnValues = array("thankyou","error","pending");	
	
	// HOOK INTO PAYMENT STATUS // V7
	if(function_exists('premiumpress_callback_paymentstatus')){
	$val = premiumpress_callback_paymentstatus($_POST);
	}
		 
	if(in_array($val, $returnValues)){ return $val; }
		...
		if(isset($_POST['customgateway']) && $_POST['customgateway'] == "blankform" ){
			...
			return $_POST['paypal_order_id'];
		
		}elseif(isset($_POST['customgateway']) && $_POST['customgateway'] != "" ){
		
			return "custom";		
			
 		}elseif(isset($_POST['cartId'])){ // worldpay
		
			return $_POST['cartId']; 
		
		}elseif(isset($_POST['BATCHID'])){ // RELEAX
		
			return $_POST['ORDER_ID'];
			
		}elseif(isset($_POST['transaction_id']) ){ // nochex
		
			return $_POST['transaction_id'];
		
		}elseif(isset($_GET['sid'])){ // 2checkout
		 
 			$order_id = $_GET['cart_order_id'];
			
			if($order_id == ""){ $order_id = $_GET['cart_id'];	}			
	...
	function ChangeListing($orderID="",$status="draft"){
		...
		if(is_numeric($d[0])){
			...
			$my_post['post_date'] = date('Y-m-d h:s'); // UPDATE THE POST DATE SO ITS RENEWED
	...
	function PaymentStatus($orderID=""){
		...
		if(isset($_GET['order_id']) && $userdata->ID == 11){ // TEST FOR ADMIN
		 
			$this->UpdateOrderStatus(5, $orderID);
			return "thankyou";	
			
		}if(isset($_GET['sid'])){ // 2checkout		  
			...
			$this->CustomGateway($_POST['customgateway']); 
			...
			if(is_numeric($emailID) && $emailID != 0){	
				...
				$result = mysql_query("SELECT order_data FROM ".$wpdb->prefix."orderdata WHERE ".$wpdb->prefix."orderdata.order_id='".strip_tags(PPTCLEAN($orderID))."' LIMIT 1", $wpdb->dbh) 
				or die(mysql_error().' on line: '.__LINE__);							
				$array = mysql_fetch_assoc($result);				
		...
		}elseif(isset($_POST['custom'])  && isset($_POST['payment_status']) 
		&& strlen($_POST['payment_status']) > 1 ){ // paypal
	...
	function InsertOrder($method="paypal",$orderID="",$status=0){
		...
		case "shopperpress": { 	
			...
			$OrderData .= "\r\n Qty: ".$qty." \r\n";
			$OrderData .= "\r\n Price: ".premiumpress_price(getProductPrice($productID)*$qty,$_SESSION['currency_code'],$GLOBALS['premiumpress']['currency_position'],1)." \r\n";
			...
			$CF=1; while($CF < 7){  
				...
				if(isset($_SESSION['ddc'][$key]['custom7']) && $_SESSION['ddc'][$key]['custom7'] !="na" && $_SESSION['ddc'][$key]['custom7'] !=""){
					...
					$OrderData .="\r\n File Attachment: ". stripslashes(get_option("imagestorage_link")).$_SESSION['ddc'][$key]['custom7']. " \r\n"; 
			...
			$O1 ="\r\n ".$_POST['form']['shippingmethod_name'].": ".premiumpress_price($_POST['form']['shippingmethod'],$_SESSION['currency_code'],$GLOBALS['premiumpress']['currency_position'],1)." \r\n";				 
			}else{
			$O1 = "";
			}
			
			
			if(isset($_POST['form']['shipping']) && strlen($_POST['form']['shipping']) > 0){
			$OrderData .= "\r\n Total Delivery Cost: ".premiumpress_price($_POST['form']['shipping'],$_SESSION['currency_code'],$GLOBALS['premiumpress']['currency_position'],1). " \r\n";	
			}
			...
			$GLOBALS['coupon'] = $_POST['form']['discount'];
             ...
             // DELIVERY ADDRESSS
			 if(!isset($_POST['form']['VATnum'])){ $_POST['form']['VATnum']=""; }
			 ...
			 $OrderData .= "\r\n --------- Requested Shipping Delivery Address --------- \r\n";	
			...
			$_POST['form']['data'] = nl2br(wpautop($OrderData));
			...
			if($emailID != "0" && strlen($emailID) > 0 ){	
			  SendMemberEmail($_POST['form']['email'], $emailID);	
			}
			...
			default: {				
					...
					$oData["currency"] 			= get_option("currency_code");
					}
					$oData["shipping"] 			= "";
					$oData["status"] 			= $status;
					$oData["username"] 			= $userdata->user_login;						
	...
	if(strtolower(PREMIUMPRESS_SYSTEM) != "shopperpress" && $orderID != ""){	
		
		$oData = explode("-",$orderID);	
		
		// CHECK IF THIS IS A MEMBERSHIP UPGRADE
		if (strpos($orderID, "MEMBERSHIP") === false) {

			// RESET THE POST DATE FOR EXPIRY				 
			$my_post = array();
			if(is_numeric($oData[0])){
				$my_post['ID'] = $oData[0];
			...
			// THIS IS A MEMBERSHHIP TO UPGRADE		
			update_user_meta($oData[0], "pptmembership_level", $oData[2]);
			update_user_meta($oData[0], "pptmembership_status", "ok");
		
		}
		 
	} // end if // not shopperpress
		...
		switch(strtolower(PREMIUMPRESS_SYSTEM)){
			...
			case "directorypress": {
				...
				$packdata = get_option("packages");
				 ...
				 update_post_meta($POSTID, "purchaseprice", $packdata[$oData[3]]['price']);
		...
		if($status ==5 && strtolower(PREMIUMPRESS_SYSTEM) != "shopperpress"){
...
global $wpdb;
	...
	if($oData["SAVE"]){
		...
		$oData[$key] =  mysql_real_escape_string($val);		
		}
