<?php
	 ...
	 * Maintainers: Andreas Pachler, Brandon Elliott..and me?
	 *
    ...
    define('ACCOUNT_TYPE', $IMAGEVALUES['stw_13']); // 'Free w/ PVP' or 'Basic or PLUS'
    ...
    define('VER', '2.0.3_dp7'); // allows us to identify known bugs and version control; DONT touch!
	...
	********************************************/	
    ...
    function getThumbnailHTML($sUrl, $aOptions = array(), $sAttribAlt = false, $sAttribClass = false, $sAttribStyle = false) {
        ...
        if (ACCOUNT_TYPE == 'pro') { // dont store values with spaces..not good idea
            $sImageTag = _getThumbnailPaid($sUrl, $aOptions, $sAttribAlt, $sAttribClass, $sAttribStyle);
        } else {
            $sImageTag = _getThumbnailFree($sUrl, $aOptions);
        }
    ...
    function _getThumbnailFree($sUrl, $aOptions) {
		// the addition of time() is critical to prevent browsers from caching the pagepix_div.js (since it changes every minute!!)
        $sCode = '<script language="javascript" type="text/javascript" src="http://www.shrinktheweb.com/scripts/pagepix_div.js?'.time().'"></script>';
        // access key (not secret key) needed for JS
        $sCode .= '<script language="javascript" type="text/javascript" id="currentScript">stw_pagepix(document.getElementById("currentScript"), "' . $sUrl . '", "' . ACCESS_KEY . '", "' . $aOptions['Size'] . '", 0);</script>';
