...
<fieldset>
<div class="titleh"> <h3>Search Box Settings</h3></div>
...
<p><b>Note.</b> Use this instead of the sidebar widget.If you are using the sidebar widget this is best unticked.</p>
...
?>
 ...
 <div class="clearfix"></div>      
<p><a href='#' onclick="return CustomSearch.get('<?php echo $prefId?>').add();" style="background:green; color:white; padding:8px;margin-left:5px;">Add Search Field</a></p>    
...
<?php
		...
		function singleFieldHTML($pref,$id,$values){
			...
			<input class='premiumpress_button' type='submit' value='Save Changes' style='color:white; float:right;' onclick=\"document.getElementById('showThisTab').value=5\" />
			
			<input type='text' name='$pref"."[label]' value='$values[label]' class='ppt-forminput' style='width:70%;'/>
			...
			<input class='premiumpress_button' type='submit' value='Save Changes' style='color:white;' onclick=\"document.getElementById('showThisTab').value=5\" /> 
			<a href='#' onclick=\"return CustomSearch.get('$prefId').remove('$id');\" style='background:red; padding:5px; color:white;margin-bottom:5px; float:right;'>Delete Search Field</a> 
			...
			if(!$CustomSearchFieldStatic['Types']){
				...
				$CustomSearchFieldStatic['Types'] = array(
					"joiner"=>array(
						...
						"CategoryJoiner" =>__( "Category List (Only categories with listings will show)",'premiumpress_advancedsearch'),
...
//						"NotEqualComparison" =>__( "Not Equal To",'premiumpress_advancedsearch'),
				...
				if(strtolower(constant('PREMIUMPRESS_SYSTEM')) == "couponpress"){
				 ...
				 $CustomSearchFieldStatic['Types'] = array(
					"joiner"=>array(
						"PostDataJoiner" =>__( "Wordpress Post Field",'premiumpress_advancedsearch'),
						"CategoryJoiner" =>__( "Category List (Only categories with coupons will show)",'premiumpress_advancedsearch'),
						"spacer1" =>__( "---------------",'premiumpress_advancedsearch'),
...
//						"NotEqualComparison" =>__( "Not Equal To",'premiumpress_advancedsearch'),
			 ...
			 $CustomSearchFieldStatic['Types'] = array(
					"joiner"=>array(
						"PostDataJoiner" =>__( "Wordpress Post Field",'premiumpress_advancedsearch'),
						"CustomFieldJoiner" =>__( "Custom Field",'premiumpress_advancedsearch'),
						"CategoryJoiner" =>__( "Category List (Only categories with products will show)",'premiumpress_advancedsearch'),
...
class DropDownField extends Field {
	...
	function getInput($name,$joiner,$fieldName=null){
		...
		return "<select name='$id'$atts class='ppt-forminput'>$options</select>";
	}
	function getConfigForm($id,$values){ 
...
class CustomSearchField extends SearchFieldBase {
	...
	function getInput($wrap=true){
		...
		$ll = $this->getLabel();
		if(strlen($ll) > 1){
		$input = "<span>".$this->getLabel()."</span> $input ";
		}else{
		$input = " $input ";
		}
