...
if(isset($_GET['page']) && $_GET['page'] == "members" && isset($_GET['edit'])){ $MYUSERID = $_GET['edit']; }else{  $MYUSERID = $userdata->ID; }
	...
	if(is_array($packagedata) && isset($packagedata['package']) ){
		...
		foreach($neworder as $package){	
			...
			$STRING .= '<div class="clearfix"></div><h4><span>'.$GLOBALS['titlec'].'</span>'.$package['name'].'</h4><div class="clearfix"></div>'; $GLOBALS['titlec']++;
			 ...
			 // START OUTPUT		
			$STRING .= '<p class="f_half left" id="field_custom_'.$i.'"><label>'.$package['name'].'</label>';
			
			// GET DEFAULT VALUE
			if(isset($GLOBALS['IS_REGISTER'])){ $FIELDVAL = $package['values']; }else{ $FIELDVAL = get_user_meta($MYUSERID, $package['key'], true); }
				...
				switch($package['type']){
					...
					case "list": {
						...
						$STRING .= '<select name="custom['.$package['key'].']" id="form_'.$package['key'].'" class="short">';
					...
					default: { 
						...
						$STRING .= '<input type="text" name="custom['.$package['key'].']" id="form_'.$package['key'].'" class="short" value="'.$FIELDVAL.'">';
...
global $wpdb, $PPT; 
	...
	$STRING = "<div id='PACKAGEAJAX'></div><input type='hidden' name='SELECTEDPACKAGEID' value='' id='SELECTEDPACKAGEID' >";
	}else{
...
function MYORDERS($user_id){
	...
	$posts = mysql_query($SQL, $wpdb->dbh) or die(mysql_error().' on line: '.__LINE__);
	  ...
	  if ($posts && mysql_num_rows($posts) > 0) {
		...
		if($thepost->order_status == 0 && strtolower(PREMIUMPRESS_SYSTEM) == "shopperpress"){
		...
		if($thepost->order_status ==5 || $thepost->order_status ==3){ 
			...
			$STRING .= "<a href='#morebox".str_replace("-","",$thepost->order_id)."' class='button green right orderinfo' style='margin-left:10px;' onclick=\"document.getElementById('moreinfodiv').value='morebox".str_replace("-","",$thepost->order_id)."';\">".$PPT->_e(array('button','13'))."</a>";
		}
		...
		// NEW COLOR BOX
		$STRING .= '<div class="gray_box" id="morebox'.str_replace("-","",$thepost->order_id).'" style="display:none"><div class="gray_box_content">';
...
global $wpdb, $PPT, $userdata; $STRING = ""; $i=1; $date_format = get_option('date_format') . ' ' . get_option('time_format');
	...
	$posts = query_posts('posts_per_page=200&post_type=ppt_alert&post_author='.$userdata->ID);
	
	if(empty($posts)){
	
	$STRING .= '<div class="yellow_box"><div class="yellow_box_content"><div align="center"><img src="'.get_template_directory_uri().'/PPT/img/exclamation.png" align="absmiddle" /> '.$PPT->_e(array('myaccount','40')).'</div><div class="clearfix"></div></div></div>';
	...
	$cat = get_the_category_by_ID( get_post_meta($post->ID, "catID", true) );	
...
global $wpdb, $PPT, $userdata; $STRING = ""; $i=1; $date_format = get_option('date_format') . ' ' . get_option('time_format'); $tcount = 0;
 
	$posts = query_posts('posts_per_page=200&post_type=ppt_message&meta_key=username&meta_value='.$userdata->user_login);
...
function SLIDER($sliderID=1){
	...
	$STRING .= '<div class="clearfix"></div><div class="clearfix" style="padding-top:10px;"></div><div class="slider9"><div class="l-rotator"><div class="screen"><noscript><!--  javascript is off --> </noscript></div><div class="thumbnails"><ul>';
	...
	foreach($sortedSlider as $slide){
			...
			'<li>
			<div class="thumb">
			'.premiumpress_image($slide,"",array('alt' => $slide->post_title,  'width' => '75', 'height' => '50', 'style' => 'max-width:70px; max-height:65px;' )).'
			...
			$STRING .=
			'<li>
			<div class="thumb"><img src="'.premiumpress_image_check($slide['s2'],'url',"&amp;w=70&amp;h=65").'" alt="'.strip_tags(str_replace('"',"",stripslashes($slide['s3']))).'" style="max-width:70px; max-height:65px;"/>
	...
	if(is_array($files)){ 
		...
		$prefix = explode(".",$file);
			...
			switch($prefix[1]){
				...
				$STRING .='<div id="flashvideo" class="marginBottom"><script type="text/javascript">
				...
				$STRING .='so.addVariable("videoAutoStart","yes");';		 
				 
				$STRING .= 'so.addVariable("videoPath","'.get_option('imagestorage_link').$file.'");';			 						 
...
if(!is_array($fields)){ return; }
	...
	if($field['name'] == "tags"){ $field['type'] = "tags"; $STRING .="<div class='clearfix'></div>"; }
		...
		$STRING .= '<p class="f_half left" id="field_'.$field['name'].'_wrapper" '.$hidden.'><label>'.$field['title'].$tooltip;
		...
		if(isset($field['editor'])){ echo $STRING; $STRING=""; } // dirty hack for wp_editor
			...
			switch($field['type']){
				...
				$STRING .= $bitbefore.'<input type="text" name="form['.$field['name'].']" id="form_'.$field['name'].'" class="'.$ex.'" tabindex="'.$GLOBALS['tabo'].'" value="'.$FIELDVALUE.'"  '.$extra.'/>';
				...
				case "category": { 
					...
					if(isset($PACKAGE_OPTIONS[$_POST['packageID']]['totalcats']) && is_numeric($PACKAGE_OPTIONS[$_POST['packageID']]['totalcats']) && isset($PACKAGE_OPTIONS[$_POST['packageID']]['a2']) && $PACKAGE_OPTIONS[$_POST['packageID']]['a2'] ==1){
	 ...
	 foreach($FieldValues as $key => $field){	
		...
		if( strlen($field['name']) > 0 && ( isset($field['pack'.$packageID]) && $field['pack'.$packageID] == 1 || get_option('pak_enabled') ==0)  ){
			...
			if(isset($field['fieldtitle']) && $field['fieldtitle'] == 1){
		 	...
		 	if($field['type'] == "textarea"){
			$STRING .= '<p class="full clearfix box"><label>'.$field['name'].'</label>';
			}else{
			$STRING .= '<p class="f_half left"><label>'.$field['name'].'</label>';
			}
			switch($field['type']){
			 case "textarea": {
			 ...
			 if(isset($_POST['action']) && !isset($data) ){ $dd = $_POST['custom'][$i]['value']; }elseif(isset($data)){ $dd = $PPT->GetListingCustom($_GET['eid'],$field['key']);  }else{ $dd = $field['value'];} 
				...
				$STRING .= '<textarea class="long" name="custom['.$i.'][value]" rows="10" cols="48" tabindex="'.$GLOBALS['tabo'].'">';				
			 ...
			 case "list": {
				...
				$STRING .= '<select name="custom['.$i.'][value]" class="short" tabindex="'.$GLOBALS['tabo'].'">';
			 ...
			 default: {	
				...
				$STRING .= '<input type="text" class="short" name="custom['.$i.'][value]" size="55" tabindex="'.$GLOBALS['tabo'].'" value="';
...
function StoreExtras($cat_id,$type="text",$return=0){ // updated 7th Jan 2012
	...
	if($type == "text"){	
		...
		$a = stripslashes(get_option("store_".$cat_id."_desc"));
		if($a == ""){ update_option("store_".$cat_id."_desc","99"); }
		if($a == "99" ) { return; }
		...
		$a = stripslashes(get_option("store_".$cat_id."_logo"));
	 
		if($a == ""){ update_option("store_".$cat_id."_logo","99"); }
		if($a == "99") { return; }
		
		$img = premiumpress_image_check($a,"full");
...
function FeaturedStores($storesID="",$order=""){
	...
	foreach($categories as $category) {	
		...
		$IMAGE = $this->StoreExtras($category->term_id,"image",1);	
		...
		$STRING .= ">";	
		if(strlen($IMAGE) > 4){ 
			$STRING .= "<img src='".$IMAGE."' style='bordr:0px; text-decoration:none;' alt='".$category->cat_name."' />";
		}	
...
global $wpdb, $PPT; $STRING = "";
	...
	if($tag == "columns-left"){	
		...
		if(isset($GLOBALS['flag-home'])){ 	
			...
			if(get_option("ppt_homepage_columns") == 3 ){ echo "left3cols left"; }else{ echo "left2cols left"; } 
			
		}elseif(isset($_GET['s']) || isset($_GET['search-class']) ){
			
			if($columnlayout == 3 ){ echo "left3cols left"; }else{ echo "left2cols left"; } 
		
		}elseif($columnlayout =="3"){ echo "left3cols left"; 		
		
		}else{  echo "left2cols left"; }
		
	}elseif($tag == "columns-right"){
 
		if(isset($GLOBALS['flag-home'])){ 
		
			if(get_option("ppt_homepage_columns") == 3 ){ echo "right3cols left"; }else{ echo "right2cols left"; } 
			
		}elseif(isset($_GET['s']) || isset($_GET['search-class']) ){
		
			if($columnlayout == 3 ){ echo "right3cols left"; }else{ echo "right2cols left"; }			
		
		}elseif($columnlayout =="3"){ echo "right3cols left"; 		
		
		}else{  echo "right2cols left"; }
	...
	$STRING = "class='padding10'"; }
 
	}elseif($tag == "columns"){	
	
		if(!isset($_GET['s']) && !isset($_GET['search-class']) ){	
		
		if(isset($GLOBALS['nosidebar-right']) && isset($GLOBALS['nosidebar-left'])){ $STRING = "full";	
		}elseif(isset($GLOBALS['nosidebar-right']) && !isset($GLOBALS['nosidebar-left'])){ $STRING = "middle2cols left";	
		}elseif(!isset($GLOBALS['nosidebar-right']) && isset($GLOBALS['nosidebar-left'])){ $STRING = "middle2cols left";	
		}else{ $STRING = "middle3cols left"; }
		
		}else{ 
		$ppt_layout_columns = $columnlayout;
		if($ppt_layout_columns == 0 ){ echo "full"; }elseif($ppt_layout_columns == 3 ){  echo "middle3cols left"; }else{ echo "middle2cols left"; }
...
function HomeCategories($TaxType="category"){ // updated Jan 7th 2012
...
/* =============================================================================
   ...
   ========================================================================== */		
	...
	if( isset($GLOBALS['premiumpress']['catID']) && is_numeric($GLOBALS['premiumpress']['catID']) ){
		...
		'taxonomy'              => $TaxType,
		'type'                  => 'post',
		'child_of'  			=>	$GLOBALS['premiumpress']['catID'],
		'hide_empty'  			=>	0,	 
		'pad_counts' 			=>	1,
		'hierarchical'          => 0,
		...
		'taxonomy'              => $TaxType,
		'type'                  => 'post',
		'orderby'				=> get_option("display_homecats_orderby"),
		'pad_counts'            => 1,
		'child_of'              => 0,
		'hide_empty'            => 0,
		'hierarchical'          => true,
	...
	if( isset($GLOBALS['premiumpress']['catID']) && is_numeric($GLOBALS['premiumpress']['catID']) && $category->parent != $GLOBALS['premiumpress']['catID'] ){ continue; }
...
function TaxonomyDisplay($cols=3,$tax='location',$all=0) {
    ...
    $list = '<div id="CouponPressStores">';
 	$tags = get_categories('order=ASC&hide_empty='.$show_empty.'&taxonomy='.$tax); 
	$groups = array();
	...
	if( $tags && is_array( $tags ) ) {
		foreach( $tags as $tag ) {
		if ($tag->parent > 0 && $all == 1) { continue; }
...
function GALLERYBLOCKDO(){
	...
	if(in_array($post->ID, $GLOBALS['galleryblockhideID'])){ continue; }
		...
		if(strtolower(constant('PREMIUMPRESS_SYSTEM')) == "directorypress" && $post->post_type == "post"){
 ...
 global $PPT;
  ...
  if ( !is_home() && !is_front_page() || is_paged() ) {
    ...
    if ( is_category() ) {
      ...
      $STRING .= $before . 'category "' . single_cat_title('', false) . '"' . $after;
...
global $wpdb,  $post, $PPT; $STRING = ""; 
	...
	$STRING = '<div class="clearfix"></div><div class="itembox '.$css.'"><h1>'.get_option("display_homecats_title").'</h1><div class="itemboxinner greybg" id="'.$idm.'"><div class="innerwrapper">'.$this->HomeCategories().'<div class="clearfix"></div></div></div></div>';
	
	}elseif($item == "recent"){ 
	
		
		if(get_option("ppt_object_recent_type") =="2" || get_option("ppt_object_recent_type") =="3"){
	
		$STRING = "";
	
		if(get_option("ppt_object_recent_type") =="2"){
	
			$GLOBALS['query_string_new'] = "orderby=ID&order=desc&posts_per_page=".get_option('ppt_object_recent_num'); $GLOBALS['galleryblockstop']=3;
		...
		case "auctionpress": {
			...
			echo  '<h1 class="title">'.get_option("ppt_object_recent_title").'</h1><div class="clearfix"></div>';	
		...
		case "shopperpress": { 
			...
			echo  '<h1>'.get_option("ppt_object_recent_title").'</h1><div class="clearfix"></div>';
			echo '<div class="itemboxinner">';
		...
		case "employeepress": { 		
			...
			echo '<div id="AJAXRESULTS"></div><div class="itembox">';
			echo '<h2>'.get_option("ppt_object_recent_title").'</h2>';
			echo '<div class="itemboxinner nopadding">';
			echo '<ol class="list">';
			echo  $this->GALLERYBLOCK();
			echo '</ol>';			
			echo  '<ul class="pagination paginationD paginationD10">'.$this->PageNavigation().'</ul><br />';	
			echo  '</div>';
			echo  '</div>';
		...
		case "directorypress": {
			...
			echo  $this->GALLERYBLOCK();
			echo '</ul></div>';
			echo  '<div class="enditembox inner"><ul class="pagination paginationD paginationD10">'.$this->PageNavigation().'</ul></div>';
			echo'</div>';			
	...
	$STRING = '<div class="clearfix"></div><div class="itembox '.$css.'"><h2>'.get_option("ppt_object_recent_title").'</h2> <div class="itemboxinner nopadding" id="homeFeaturedList"><ul>';					
	
		foreach ($postslist as $loopID => $p){
			$STRING .= '<li>'.premiumpress_image($p->ID,"",array('link' => true,'alt' => $p->post_title, 'width' => '100', 'height' => '100', 'style' => 'auto' )).'			 
			<h3><a href="'.get_permalink($p->ID).'" title="'.$p->post_title.'">'.$p->post_title.'</a></h3><p>'.strip_tags($p->post_excerpt).'</p>
			</li>';
		}    
	$STRING .= '</ul></div></div>'; 
	...
	$ThisQuery = 'posts_per_page=25&orderby=ID&order=desc';
	}else{
	$ThisQuery = get_option("ppt_object_carousel_custom");
	}
	$postslist = query_posts($ThisQuery);
	if(!empty($postslist)){
...
global $userdata, $post;
	...
	if(isset($GLOBALS['claim_email']) && $GLOBALS['claim_email'] !=""  && $post->post_author == 1 ){ //  && $GLOBALS['user_info']->wp_user_level == 10  
 
     
	}else{ 

	$STRING = '<div class="itembox"><h2>'.$PPT->_e(array('add','52')).'</h2><div class="itemboxinner greybg"> ';    
		...
		if($img == ""){
			...
			$img = "<img src='".get_option('imagestorage_link').$img."' class='photo' alt='user ".$GLOBALS['authorID']."' />";
		}
        ...
        <p><img src="'.IMAGE_PATH.'icon2.png" alt="send email" align="middle" /> <a href="'.get_option("messages_url").'?u='.get_the_author_meta( 'user_nicename', $GLOBALS['authorID']).'" rel="nofollow">'.str_replace("%a",get_the_author_meta( 'user_nicename', $GLOBALS['authorID']),$PPT->_e(array('author','1'))).'</a></p>
...
if($title){ return $pages_text; }
					...
					if ($start_page >= 2 && $pages_to_show < $max_page) {
						...
						if($paged > 4 && $paged < $max_page){
						
						$return .= '<li><a href="'.esc_url(get_pagenum_link($paged-5)).'" class="first" title="'.$first_page_text.'"><<</a></li>';
