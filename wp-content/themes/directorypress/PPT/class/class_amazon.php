...
function AmazonAutoUpdaterTool(){
		...
		print "<h2>Showing 20 products per page, starting at post ID ".$LASTID."</h2><p>Refresh the page again to view the next 20</p>";
...
if($change == 1){
echo "<div style='padding:10px; background:#aadcfb; border:1px solid #1d78b0; margin-bottom:10px; clear:both;'>";
}else{
echo "<div style='padding:10px; background:#e7ffcc; border:1px solid #88c446; margin-bottom:10px; clear:both;'>";

}			
echo '<img src="'.premiumpress_image($value->ID,"full") .'" style="float:left; max-width:80px; max-height:80px; padding-right:30px; padding-bottom:20px;">';
			
echo "<h3>Checking for product changes...</h3> 
...
<p>(<a href='".get_bloginfo('url')."/?p=".$value->ID."' target='_blank'>view product</a> / <a href='post.php?post=".$value->ID."&action=edit' target='_blank'>edit product</a>)</p> <p>";
					...
					print " price is now ".$data['price']." old price is ".$data['old_price'];
					...
					print " </p><div style='clear:both'></div></div>";
