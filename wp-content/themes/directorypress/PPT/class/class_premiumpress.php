...
function Image($data1){
	...
	$height 		= 	(isset($data1[2]['height']) && $data1[2]['height'] 	!= "" ? 'class="'.strip_tags($data1[2]['height']).'"' : ""); // height
	...
	if($IMAGEVALUES['format'] =="1" && strpos($img, "http") === false && !$useAPI && $img != ""){
		...
		$img = $this->ImageCheck($IMAGEVALUES['noimage']);
			...
			if(isset($img_bits[2])){ $img_ext = $img_bits[2]; $img_name = $img_bits[0]."".$img_bits[1];  }else{ $img_ext = $img_bits[1]; $img_name = $img_bits[0]; }
...
if(!is_array($data)){ return $data; }
	...
	if(is_numeric($price)){		
		if($price > 999 && $skip == 0){			
			$price = @number_format($price,$digs, $seperator, ',');		
...
function Expired($data){ 
	...
	if(strtolower(constant('PREMIUMPRESS_SYSTEM')) == "shopperpress"){  return false; } // no expiry tool for SP
	...
	if(strtolower(constant('PREMIUMPRESS_SYSTEM')) == "couponpress"){ 
		...
		if( date("Y-m-d h:i:s") > $AHHH ){ 
		 //die(date("Y-m-d h:i:s") .">". $AHHH);	
				...
				// SEND EMAIL TO LET THE AUCTION OWNER KNOW ITS FINISHED
				$emailID = get_option("email_auction_end");					 
				if(is_numeric($emailID) && $emailID != 0){
					SendMemberEmail($post_data->post_author, $emailID);
				}
...
global $wpdb, $post; $s = "";
	...
	if(is_numeric($data[0])){ // BANNER ZONE
		...
		return stripslashes(get_option("advertising_zone_".$data[0]));	
		...
		if($data[1] == 1){ return $s; }else{ echo $s; }		
...
if(!is_numeric($cat_id)){ return $data; }
 
	global $wpdb;
	
	if($type == "text"){
	 
		$a = stripslashes(get_option("cat_extra_text_".$cat_id));		
		if($a == ""){ update_option("cat_extra_text_".$cat_id,"99"); }
		if($a == "99" ) { return; }
		...
		$a = stripslashes(get_option("cat_extra_image_".$cat_id));
	 
		if($a == ""){ update_option("cat_extra_image_".$cat_id,"99"); }
		if($a == "99") { return; }
		
		$img = $this->ImageCheck($a,"full");
...
/* =============================================================================
   ...
   ========================================================================== */
		...
		foreach(get_cat_hierchy(0,$args) as $category) {
		
		if($showExtraPrice = 1){ $price  = get_option('CatPrice_'.$category->cat_ID); }else{ $price  = ""; }
		if($price == ""){ $EXTRA = ""; }else{ $EXTRA = " + ".$CCode.$price; }	
		...
		$STRING .= '</option>';
			...
			foreach($category->children as $child_category){	
				...
				if($showExtraPrice = 1){ $price  = get_option('CatPrice_'.$child_category->cat_ID); }else{ $price  = ""; }
				if($price == ""){ $EXTRA = ""; }else{ $EXTRA = " + ".$CCode.$price; }	
		 
				$STRING .= '<option value="'.$child_category->cat_ID.'" ';
				if( ( is_array($id) && in_array($child_category->cat_ID,$id) ) ||  ( !is_array($id) && $id == $child_category->cat_ID ) ){
				$STRING .= 'selected=selected';
				}
				$STRING .= '> -- ';
				...
				$STRING .= '</option>';	
					...
					foreach( $child_category->children as $child_category1){	
						...
						if($showExtraPrice = 1){ $price  = get_option('CatPrice_'.$child_category1->cat_ID); }else{ $price  = ""; }
						if($price == ""){ $EXTRA = ""; }else{ $EXTRA = " + ".$CCode.$price; }	
				 
						$STRING .= '<option value="'.$child_category1->cat_ID.'" ';
						if( ( is_array($id) && in_array($child_category1->cat_ID,$id) ) ||  ( !is_array($id) && $id == $child_category1->cat_ID ) ){
						$STRING .= 'selected=selected';
						}
						$STRING .= '> ---- ';
								 
						$STRING .= $child_category1->cat_name;
						$STRING .= $EXTRA;
						if($ShowCatCount =="yes"){
							$STRING .= " (".$child_category1->count.')';
						}			 
						$STRING .= '</option>';	
							...
							foreach( $child_category1->children as $child_category2){	
								...
								if($showExtraPrice = 1){ $price  = get_option('CatPrice_'.$child_category2->cat_ID); }else{ $price  = ""; }
								if($price == ""){ $EXTRA = ""; }else{ $EXTRA = " + ".$CCode.$price; }	
						 
								$STRING .= '<option value="'.$child_category2->cat_ID.'" ';
								if( ( is_array($id) && in_array($child_category2->cat_ID,$id) ) ||  ( !is_array($id) && $id == $child_category2->cat_ID ) ){
								$STRING .= 'selected=selected';
								}
								$STRING .= '> ------ ';
										 
								$STRING .= $child_category2->cat_name;
								$STRING .= $EXTRA;
								if($ShowCatCount =="yes"){
									$STRING .= " (".$child_category2->count.')';
								}			 
								$STRING .= '</option>';	
								
									foreach( $child_category2->children as $child_category3){	
									 
										if($showExtraPrice = 1){ $price  = get_option('CatPrice_'.$child_category3->cat_ID); }else{ $price  = ""; }
										if($price == ""){ $EXTRA = ""; }else{ $EXTRA = " + ".$CCode.$price; }	
								 
										$STRING .= '<option value="'.$child_category3->cat_ID.'" ';
										if( ( is_array($id) && in_array($child_category3->cat_ID,$id) ) ||  ( !is_array($id) && $id == $child_category3->cat_ID ) ){
										$STRING .= 'selected=selected';
										}
										$STRING .= '> -------- ';
												 
										$STRING .= $child_category3->cat_name;
										$STRING .= $EXTRA;
										if($ShowCatCount =="yes"){
											$STRING .= " (".$child_category3->count.')';
										}			 
										$STRING .= '</option>';	
										
											foreach( $child_category3->children as $child_category4){	
											 
												if($showExtraPrice = 1){ $price  = get_option('CatPrice_'.$child_category4->cat_ID); }else{ $price  = ""; }
												if($price == ""){ $EXTRA = ""; }else{ $EXTRA = " + ".$CCode.$price; }	
										 
												$STRING .= '<option value="'.$child_category4->cat_ID.'" ';
												if( ( is_array($id) && in_array($child_category4->cat_ID,$id) ) ||  ( !is_array($id) && $id == $child_category4->cat_ID ) ){
												$STRING .= 'selected=selected';
												}
												$STRING .= '> ---------- ';
														 
												$STRING .= $child_category4->cat_name;
												$STRING .= $EXTRA;
												if($ShowCatCount =="yes"){
													$STRING .= " (".$child_category4->count.')';
												}			 
												$STRING .= '</option>';	
												
												
													foreach( $child_category4->children as $child_category5){	
														 
															if($showExtraPrice = 1){ $price  = get_option('CatPrice_'.$child_category5->cat_ID); }else{ $price  = ""; }
															if($price == ""){ $EXTRA = ""; }else{ $EXTRA = " + ".$CCode.$price; }	
													 
															$STRING .= '<option value="'.$child_category5->cat_ID.'" ';
															if( ( is_array($id) && in_array($child_category5->cat_ID,$id) ) ||  ( !is_array($id) && $id == $child_category5->cat_ID ) ){
															$STRING .= 'selected=selected';
															}
															$STRING .= '> ------------ ';
																	 
															$STRING .= $child_category5->cat_name;
															$STRING .= $EXTRA;
															if($ShowCatCount =="yes"){
																$STRING .= " (".$child_category5->count.')';
															}			 
															$STRING .= '</option>';	
														}
...
global $wpdb; global $userdata; get_currentuserinfo();
	...
	$DefaultFields = array("title","description","short","tags");
		...
		if(get_option("pak_auto_edit") == 1 && $type == "edit"){		
			$my_post['post_status'] 	= "publish";		
		}
...
return "<div style='padding:10px;border:1px dotted #ccc; background:#efefef; font-size:11px; color:#444;line-height:20px;height:100%;'>
<b>".$name."</b>
	...
	function IMPORTSWITCH($period){ 
		...
		switch(strtolower(constant('PREMIUMPRESS_SYSTEM'))){
			...
			case "shopperpress": {
				...
				if(get_option('enabled_amazon_updater') == "yes"){ $obj->AmazonAutoUpdaterTool(); }
...
function MEMBERSHIPS($period){
	...
	if($period == "twicedaily" || $period == "daily"){
...
function ICODESIMPORT($period="hourly",$debug=false){
	...
	if(get_option("icodes_autoimport_offers") == "1"){ 
		...
		$QueryString .= "UserName=".get_option('icodes_subscription_username');
		$QueryString .= "&SubscriptionID=".get_option('icodes_subscriptionID')."&Relationship=joined";
		$QueryString .= "&RequestType=Offers&Action=New&Hours=24&PageSize=50";
	...
	if($array['total'] == 0){ 
		...
		if($cat == "setup"){
			 ...
			 if ( is_term( $code['category'] , 'category' ) ){				 
				...
				$term = wp_insert_term("Download Products", 'category', $args);
				//$cat = $cat_id['term_id'];
				$cat = $term->term_id;
			 }
		}
