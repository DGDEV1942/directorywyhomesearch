...
case "WishList":{ 
	...
	if($_GET['act'] == "add"){
		...
		print "<div class='green_box'><div class='green_box_content'><img src='".get_template_directory_uri()."/PPT/img/admin/ok.png' align='absmiddle' /> ".get_the_title($_GET['itemID'])." ".$PPT->_e(array('validate','15'))." <a href='".get_option("dashboard_url")."' style='text-decoration:underline;'>".$PPT->_e(array('validate','16'))."</a> </div></div>";
		
		// ELSE WE NEED TO GIVE ERROR MSG TO USE		
		}else{
		
		print "<div class='red_box'><div class='red_box_content'><img src='".get_template_directory_uri()."/PPT/img/cross.png' align='absmiddle' /> ".get_the_title($_GET['itemID'])." ".$PPT->_e(array('validate','17'))." <a href='".get_option("dashboard_url")."' style='text-decoration:underline; color:#7c0f06;'>".$PPT->_e(array('validate','16'))."</a></div></div>";
...
$STRING .= '<div  id="country_city_state_select'.$nf[2].'_innerdiv"><select name="'.$id.$idx.'" ';
...
echo $ThemeDesign->MakeStatusMeter($_GET['id'],"small");
	...
	case "UserActionForm": {
		...
		if($_GET['act'] == "login"){
			...
			if ( is_wp_error($user) ){
				...
				print "<p class='CheckoutBtn'><a href='javascript:void(0);' onclick=\"jQuery('#step1box').hide();jQuery('#step2box').show();;\" style='color:white;'>".$PPT->_e(array('button','10'))."</a></p>";
...
case "changestate": {
	...
	if(in_array($_GET['current'],$state_included)){
		...
		print  '<p class="f_half left"><label class="col1">'.$PPT->_e(array('myaccount','30')).'</label><input type="text" name="form[state]" class="short" size="35" maxlength="200" id="state" tabindex="11" /></p>';
		}
	...
	$PASSCODE = '<h1>%title%</h1>
			...
			<ul class="item_stats" style="margin:0;">
			 ...
			 <div class="clearfix"></div>
			  ...
			  <br /><p><a href="%link%" class="btn">'.$PPT->_e(array('button','13')).'</a></p>';
...
print  '<img src="'.premiumpress_image_check(trim($_GET['d']),"full").'" style="max-width:500px;max-height:450px;">';
